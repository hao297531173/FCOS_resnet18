...
def evaluate_coco(generator, model, threshold=0.05):
    ...
    results = []
    image_ids = []
    for index in tqdm(range(len(generator)-4940)):
        img, gt_boxes, gt_labels, scale = generator[index]
        # run network
        scores, labels, boxes = model(img.unsqueeze(dim=0).to(device))
...
if __name__ == "__main__":
    ...
    generator = COCOGenerator(img_path, ann_path)
    model = network.FCOS_eval("./model/fcos_res18.pth", gpu=0).to(device)

    evaluate_coco(generator, model, threshold=0.5)
